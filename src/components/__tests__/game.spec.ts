...
describe(('Game Construction'), ()=> {
  it('should assign correct values', () => {
    ...
    expect(game.current_hand).toEqual(new Hand({round_wind: Winds.EAST, hand: 1, honba: 0, riichi_sticks: 0}))
...
describe(('Game SetUpNextHandOrFinishGame'), () => {
  it('Should move forward on draw dealer noten', () => { 
    ...
    expect(game.players.GetPlayers([Winds.EAST, Winds.SOUTH, Winds.WEST, Winds.NORTH]).map((p) => p.current_wind)).toEqual([
      Winds.NORTH, Winds.EAST, Winds.SOUTH, Winds.WEST
    ])
